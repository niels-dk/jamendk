...
function route(string $uri): void
{
    $routes = [
        ...
        '/api/visions/([A-Za-z0-9]{6,16})/groups'            => ['media','groups_index'],    // GET
		
		// Groups (creator-scoped, used by media library)
		//'/api/visions/([A-Za-z0-9]{6,16})/groups'        => ['groups','list'],   // GET
		'/api/visions/([A-Za-z0-9]{6,16})/groups:create' => ['groups','create'], // POST
		...
		// Groups – list/create (creator-scoped under a vision)
		'/api/visions/([A-Za-z0-9]{6,16})/tags'         => ['media','tags_list'],    // GET
		'/api/visions/([A-Za-z0-9]{6,16})/tags:create'  => ['media','tags_create'],  // POST

		// Per-media assignment
		'/api/media/([0-9]+)/group'                     => ['media','group_update'], // POST
		'/api/media/([0-9]+)/tags'                      => ['media','update_tags'],  // POST (CSV or array)
		...
		// Tags (creator-scoped)
		'/api/tags'											 => ['media','tags_index'],      // GET
		'/api/media/([0-9]+)/tags'							 => ['media', 'tags_update'], // POST -> set tags for a media
