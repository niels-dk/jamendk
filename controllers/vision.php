<?php
    ...
    class vision_controller
    {
        ...
        /** POST /visions/store (legacy create) */
        ...
        /** State actions (archive/unarchive/restore/etc.) */
        ...
        /** POST /api/visions/update-basics */
        public static function updateBasics(): void
        {
            ...
            $stmt = $db->prepare("INSERT INTO vision_presentation (vision_id, $columns)
                    VALUES (?, $placeholders)
        ...
        /**
         * POST /api/visions/{slug}/save
         * AJAX endpoint to persist title, description and anchors.
         */
        public static function ajax_save(string $slug): void
        {
            ...
            if (array_key_exists('title', $payload)) {
                ...
                $fieldsToUpdate['title'] = $t === '' ? null : $t;
                $result['title'] = true;
            }

            // Description – sanitize HTML, remove <img>
            if (array_key_exists('description', $payload)) {
                $desc = trim((string)$payload['description']);
                if ($desc === '') {
                    $fieldsToUpdate['description'] = null;
                } else {
                    $allowed = '<b><i><ul><ol><li><a><p><h1><h2><h3><br>';
                    $clean   = strip_tags($desc, $allowed);
                    $fieldsToUpdate['description'] = $clean;
                }
                $result['description'] = true;
            }

            // Persist scalar fields if any
            if ($fieldsToUpdate) {
                vision_model::partialUpdate($db, (int)$vision['id'], $fieldsToUpdate);
            }
            ...
            // Flip status from draft → active if necessary
            if (($vision['status'] ?? 'draft') === 'draft') {
                $newTitle = array_key_exists('title', $fieldsToUpdate) ? $fieldsToUpdate['title'] : $vision['title'];
                $newDesc  = array_key_exists('description', $fieldsToUpdate) ? $fieldsToUpdate['description'] : $vision['description'];
                if ($newTitle || $newDesc) {
